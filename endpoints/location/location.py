...
def query_mysql(campsite_id) -> dict:
    result: dict = None
    destination_table: str = "Campsites_Location"
    ...
    try:
        with conn.cursor() as cursor:
            cursor.execute(f"SELECT campsite_country, campsite_state, campsite_city FROM {destination_table} WHERE campsite_id = {campsite_id} ")
...
def transform(records: list) -> list:
    ...
    for record in records:
        data: dict = {}
        data['campsite_country'] = record[0]
        data['campsite_state'] = record[1]
        data['campsite_city'] = record[2]
