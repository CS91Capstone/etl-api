...
def query_mysql(campsite_id) -> dict:
    result: dict = None
    destination_table: str = "Campsites_Type"
    ...
    try:
        with conn.cursor() as cursor:
            cursor.execute(f"SELECT campsite_type, type_of_use FROM {destination_table} WHERE campsite_id = {campsite_id} ")
...
def transform(records: list) -> list:
    ...
    for record in records:
        data: dict = {}
        data['campsite_type'] = record[0]
        data['type_of_use'] = record[1]
