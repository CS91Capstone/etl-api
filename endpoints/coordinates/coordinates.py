...
def query_mysql(campsite_id) -> dict:
    result: dict = None
    destination_table: str = "Campsites_Coordinates"
    ...
    try:
        with conn.cursor() as cursor:
            cursor.execute(f"SELECT campsite_longitude, campsite_latitude FROM {destination_table} WHERE campsite_id = {campsite_id} ")
...
def transform(records: list) -> list:
    ...
    for record in records:
        data: dict = {}
        data['campsite_longitude'] = record[0]
        data['campsite_latitude'] = record[1]
